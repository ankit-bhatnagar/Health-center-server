...
int main(int argc, char **argv)
{
	...
	strcpy(file_name,"doc1.txt");
	fp = fopen(file_name,"r"); // read mode
	
	struct hostent *lh = gethostbyname("nunki.usc.edu"); //get IP address
	
	printf("\nPhase 3: Doctor 1 has a static UDP port %d and IP address %s",SERVICE_PORT,inet_ntoa(*((struct in_addr *)lh->h_addr)));
	...
	while(1)
  {
		...
		if (recv_size > 0) {
			buf[recv_size] = 0;
			printf("\nPhase 3: Doctor 1 receives the request from the patient with port number %d and insurance plan  %s",(int)ntohs(((struct sockaddr_in *)(&remaddr))->sin_port), buf);
		}
		else
		...
		if (sendto(fd,new_buf, strlen(new_buf), 0, (struct sockaddr *)&remaddr, addrlen) < 0)
			perror("sendto");
		printf("\nPhase 3: Doctor 1 has sent estimated price %s$ to patient with port number %d",new_buf,(int)ntohs(((struct sockaddr_in *)(&remaddr))->sin_port));	
  }	
	close(fd);
	printf("\n");
	return 0;
