...
int main(int argc, char *argv[])
{
	...
	printf("\nPhase 1: Patient 2 has TCP port number %u and IP address %s",(unsigned)ntohs(adr_inet.sin_port),inet_ntoa(*((struct in_addr *)lh->h_addr)));
	strcpy(ipp,inet_ntoa(*((struct in_addr *)lh->h_addr)));
    
	fp = fopen("patient2.txt","r"); // read mode
	fflush(stdout);
   if( fp == NULL )
   {
  ...
  fclose(fp);
    ...
    printf("\nPhase 1: Authentication request from Patient 2 with username %s and password %s has been sent to the Health center server.",a,b);
	fflush(stdout);
	if ((numbytes = recv(sockfd, buf, max_size-1, 0)) == -1) 
	    perror("recv");
	buf[numbytes] = '\0';
		printf("\nPhase 1: Patient 2 authentication result: %s",buf);
	int succ,fail;
	...
	if(check==0)
	{
		...
		do{ 
		   ...
		   if(app_choice>=1 && app_choice<=6)
			...
			if(avail_check!=0)
			{
			 printf("\nPhase 2: The requested appointment is available and reserved for Patient 2. The assigned doctor port number is %s",buf);
			 strcpy(doc_info,buf);
			}
			else
			{
	...
	*/
	...
	strcpy(file_name,"patient2insurance.txt");
	fp = fopen(file_name,"r"); // read mode
	
	fflush(stdout);
   if( fp == NULL )
   {
      perror("Error while opening the patient2 file.\n");
      exit(EXIT_FAILURE);
   }
   ...
   if(insur_info[len-1] == '\n')
	...
	printf("\nPhase 3: Patient 2 has a dynamic UDP port number %u and IP address %s",(unsigned)ntohs(adr_inet.sin_port),ipp);
	...
	/* insurance info sent to doctor port!! */
		...
		printf("\nPhase 3: The cost estimation request from Patient 2 with insurance plan %s has been sent to the doctor with port number %d and IP address %s",insur_info,SERVICE_PORT,ipp);
		...
		recvlen = recvfrom(fd, buf, maxbuf, 0, (struct sockaddr *)&remaddr, &slen);
                if (recvlen >= 0) {
					...
					printf("\nPhase 3: Patient 2 receives %s$ estimation cost from doctor with port number %d and name %s",buf,SERVICE_PORT,doc_name);	
                }
